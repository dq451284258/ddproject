    ...
    <div class="calendar">
            ...
            <div class="calendar-info">
                ...
                <div class="month"> 
                    <div class="month-inner" :style="{'top':-(this.month*20)+'px'}">
                        <span v-for="(m, mindex) in months" :key="mindex">{{m}}月</span>
                        <!-- <span v-for="m in months">{{m}}月</span> -->
                    </div>
                </div>
        ...
        <thead>
            <tr>
                ...
                <td v-for="(week, windex) in weeks" :key="windex"  class="week">{{week}}</td>
            </tr>
        </thead>
        <tbody>
        <tr v-for="(day,k1) in days" :key="k1" style="{'animation-delay',(k1*30)+'ms'}">
            <td v-for="(child,k2) in day" :key="k2" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)">
                ...
                <div :class="{cgreen:child.green,'circle':child.circleRed}"></div>
        ...
        <div class="calendar-years" :class="{'show':yearsShow}">
            <span v-for="(y, yindex) in years" :key="yindex"  @click.stop="selectYear(y)" :class="{'active':y==year}">{{y}}</span>
...
//  	dingAuth();
        this.init();
        this.fit(); 
        this.circle.startdate = this.year + '-' + this.intMonth + '-' + '01';
        this.circle.enddate = this.year + '-' + this.intMonth + '-' + this.lastDay;   
        // 设置时间 
        localStorage.setItem('startdate', this.circle.startdate);   
        localStorage.setItem('enddate', this.circle.enddate);   

        //获取绿点数据
        this.getCircle(); 
//      console.log(this.circleData);  
//      console.log(this.circle.startdate);
//      console.log(this.circle.enddate);
    },
    methods: {
    	//适配
	    fit(){
		...
		getCircle() { 
	        ...
	        this.$emit('sdata',this.circleData);
        },
        //红绿点
		choose(){
			let _tag = false;
			let _this =this;
			_this.circleData.map((item, index) => { 
				// console.log(item)				
...
//                  
                ...
                if (this.range) { // 范围
                    ...
                    if (this.rangeBegin.length > 0) {
                        ...
                        if (beginTime <= stepTime && endTime >= stepTime) {
                           ...
                           if (month < 10) {
                            ...
                            if (i < 10){ 
								options.all = this.year + '-' + monthAll + '-' + '0' + i;
							}else{
								options.all = this.year + '-' + monthAll + 1 + '-' + i;
							} 
                    ...
                    if (i < 10){ 
						allDay = this.year + '-' + monthAll + '-' + '0' + i;
					}else{
						allDay = this.year + '-' + monthAll + '-' + i;
					}
                    // 判断是否选中
                    if(this.value.filter(v => {return this.year === v[0] && this.month === v[1]-1 && i === v[2] }).length>0 ){                    	
                        options = Object.assign({day: i,selected:true,circleRed:true,all:allDay},this.getLunarInfo(this.year,this.month+1,i),this.getEvents(this.year,this.month+1,i))
                    }else{
                        options = Object.assign({day: i,selected:false,circleRed:true,all:allDay},this.getLunarInfo(this.year,this.month+1,i),this.getEvents(this.year,this.month+1,i))
                        if (this.begin.length>0) {
                            let beginTime = Number(new Date(parseInt(this.begin[0]),parseInt(this.begin[1]) - 1,parseInt(this.begin[2])))
                            if (beginTime > Number(new Date(this.year, this.month, i))) options.disabled = true
                        }
                    ...
                    if (parseInt(seletSplit[0]) == this.year && parseInt(seletSplit[1]) - 1 == this.month && parseInt(seletSplit[2]) == i) {
                        ...
                        temp[line].push(Object.assign(
                            {day: i,selected: true,circleRed:true,all:allDay},
                            this.getLunarInfo(this.year,this.month+1,i),
                            this.getEvents(this.year,this.month+1,i),
                        ))
                     ...
                     // 没有默认值的时候显示选中今天日期
                        ...
                        let options = Object.assign(
                            {day: i,selected:false,circleRed:true,all:allDay},
                            this.getLunarInfo(this.year,this.month+1,i),
                            this.getEvents(this.year,this.month+1,i),
                        )
                        if (this.begin.length>0) {
            ...
            this.days = temp;
            //  console.log(temp) 
            this.circleData = temp;
            this.lastDay = lastDateOfMonth;
            this.intYear = this.year;
            this.intMonth = this.month + 1;
            if(this.intMonth < 10){
            	this.intMonth = '0' + this.intMonth;
            }else{
