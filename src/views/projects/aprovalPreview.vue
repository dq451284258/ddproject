              ...
              <el-form-item label="项目阶段:" class="readonly-text">
                <div class="worktime-stage" v-for="(s, index) in projectForm.steping" :key="index">
                  ...
                  <div class="stage-raw">
                    ...
                    <div class="content">
                      <updownloadFile v-if="projectForm.loading" :files="s.files" :type="'stage'" @addFiles='addFiles' :index="index" :canup="authority.projectStepEdit ? 'y' : 'n'" />
                    </div>
                  </div>
                </div>
              </el-form-item>
              <el-form-item label="终止协议" class="readonly-text" v-show="projectForm.projType == 2">
                <updownloadFile v-if="projectForm.loading" :files="projectForm.overFiles" :type="'over'" @addFiles='addOverFiles' :canup="authority.projectStatusTerminate ? 'y' : 'n'" />
              </el-form-item>
              <el-form-item label="" class="readonly-text">
                <div class="el-form-button">
                  <el-button @click="overProject()" :loading="submitLoading" v-show="authority.projectStatusTerminate">终止项目</el-button>
                  <el-button @click="endProject()" :loading="endLoading" v-show="authority.projectStatusDone">项目完工</el-button>
                  <el-button type="primary" @click="projectHanding()" :loading="saveLoading" v-show="authority.projectStatusRecover">开启项目</el-button>
                  <el-button type="primary" @click="projectHanding()" :loading="saveLoading" v-show="authority.projectStatusSuspend">暂停项目</el-button>
                </div>
              </el-form-item>
            </div>
          </div>
...
<script>
  ...
  import overUploadBox from '@/components/overUploadBox'
  ...
  export default {
    ...
    methods: {
      ...
      overProject() {
        if(!this.projectForm.overFiles.length) {
          this.$message({
            message: '请上传终止协议',
            type: 'info'
          })
        ...
        let files = []
        for(let i = 0; i < this.projectForm.overFiles.length; i ++) {
          files.push(this.projectForm.overFiles[i].url+'?name='+this.projectForm.overFiles[i].name)
        }
        let filestr = files.join(',')
        this.submitLoading = true
        editStatus(this.projectForm.id, 4, filestr).then(res => {
          if(res.result === 'success') {
      ...
      projectHanding() {
        let s = this.projectForm.status == 209 ? 3 : 2
        this.saveLoading = true
        editStatus(this.projectForm.id, s, '').then(res => {
          if(res.result === 'success') {
      ...
      getDetail() {
        getDetail(this.$route.params.id).then(res => {
          ...
          if(this.$route.params.id == 0) {
            ...
            this.projectForm.overFiles = res.data.base.terminal_agreement ? filesToArray(res.data.base.terminal_agreement) : []
            this.projectForm.status = res.data.base.status
            if(res.data.base.status == 210 || res.data.base.status == 208) {
              this.projectForm.showbtn = false
            }else {
              this.projectForm.showbtn = true
            }
      ...
      addFiles(files, index, type) {
        editFiles(this.projectForm.steping[index].id, files).then(res => {
          if(res.result === 'success') {
            ...
            this.projectForm.steping[index].files.splice(this.projectForm.steping[index].files.length, 1)
            this.$message({
              message: res.data.errmsg,
              type: 'info'
            })
      ...
      addOverFiles(files, type) {
        this.projectForm.overFiles = files
      },
      getNameById() {
